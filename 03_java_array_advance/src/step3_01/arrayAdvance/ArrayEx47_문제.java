package step3_01.arrayAdvance;

import java.util.Random;
import java.util.Scanner;

// # 소코반

public class ArrayEx47_문제 {

	public static void main(String[] args) {
		
		Scanner scan = new Scanner(System.in);
		Random ran = new Random();
		
		final int SIZE = 7;
		final int PLAYER = 2;
		final int BALL = 3;
		final int GOAL = 7;
		final int WALL = 9;
		
		int[][] map = new int[SIZE][SIZE];
		
		int wallCount = 0;
		int pY = 0;
		int pX = 0;
		int ballY = 0;
		int ballX = 0;
		int goalY = 0;
		int goalX = 0;
		
		// 벽 설치
		System.out.print(">>>설치할 벽의 개수를 입력하세요 : ");
		wallCount = scan.nextInt();
		
		
		// 랜덤으로 벽 설치..
		
		/*for (int i = 0; i < wallCount; i++) {
			int xWall = ran.nextInt(SIZE);
			int yWall = ran.nextInt(SIZE);
		
			if (map[xWall][yWall] == )
			map[xWall][yWall] = WALL;
		}*/
		
		int temp = wallCount;
		while (temp != 0) {
			int rY = ran.nextInt(SIZE);
			int rX = ran.nextInt(SIZE);
			
			if (map[rY][rX] == 0) {
				map[rY][rX] = WALL;
				temp = temp - 1;
			}
		}
		
		// 공 설치 
		
		/*while (true) {
			ballY = ran.nextInt(SIZE
			ballX = ran.nextInt();
			if (map[ballY][ballX] == 0) {
				map[ballY][ballX] = BALL;
				break;
			}
		}*/
		
		
		
		
		
		
		
		
		while (true) {
			int rY = ran.nextInt(SIZE - 2) + 1; // ball 은 처음과 끝부분에 변두리에 있으면 안되므로 처음과 끝을 제외한 랜덤값..
			int rX = ran.nextInt(SIZE - 2) + 1;
			
			if(map[rY][rX] == 0) { //랜덤으로 좌표를 지정했을 때 값이 아무것도 없으면 그떄서야 좌표 확정.
				map[rY][rX] = BALL;
				ballY = rY; 
				ballX = rX;
				break;
			}
		}
		
		// 골대 설치
		
		
		/*while (true) {
			int gY = ran.nextInt(SIZE);
			int gX = ran.nextInt(SIZE);
			
			if (map[gY][gX] == 0) {
				goalY = gY;
				goalX = gX;
				break;
			}
		}*/
	
		while (true) {
			int rY = ran.nextInt(SIZE);
			int rX = ran.nextInt(SIZE);
			
			if (map[rY][rX] == 0) {
				map[rY][rX] = GOAL;
				goalY = rY;
				goalX = rX;
				break;
			}
		}
		
		// 화면 출력
		for (int i = 0; i < SIZE; i++) {
			for (int j = 0; j < SIZE; j++) {
				System.out.print(map[i][j] + " ");
			}
			System.out.println();
		}
		
		// 플레이어 배치
		
		
		
		/*while (true) {
			pY = scan.nextInt();
			pX = scan.nextInt();
			
			if (map[pX][pY] == 0) {
				map[pY][pX] = PLAYER;
				break;
			}		
		*/

		
		
		while (true) {
			System.out.println(">>>플레이어의 위치를 선택하세요.");
			System.out.print("y좌표 입력 : ");
			pY = scan.nextInt();
			
			System.out.print("x좌표 입력 : ");
			pX = scan.nextInt();
			
			if (map[pY][pX] == 0) {
				map[pY][pX] = PLAYER;
				break;
			}
		}
		
		// 게임 시작
		
	/*	while (true) {
			for (int i = 0; i < SIZE; i++) {
				for (int j = 0; j < SIZE; j++) {
					if (map[i][j] == 0) System.out.print("0 ");
					else if (map[i][j] == 2) System.out.print("옷 ");
					else if (map[i][j] == 9) System.out.println("! ");
					else if (map[i][j] == 3) System.out.println("* ");
					else if (map[i][j] == 7) System.out.println("G "); 
				}
				System.out.println();
			}
			
			System.out.println("상(1)하(2)좌(3)우(4) 입력 : ");
			int num = scan.nextInt();
			
			if (num == 1) {
				pY--;
				if (map[pY][pX] == 9 || map[pY][pX] == 7) {
					return;
				}
				else if (map[pY][pX] == 3){ 
					ballY--;
					pY--;
					if (ballY > 0){
						map[ballY][ballX] = BALL; 
						map[pY][pX] = PLAYER;
					}
					else return;
				}
				else {
					pY--;
					if (pY > 0) {
						map[pY][pX] = PLAYER;
					}
				}
			}
			else if (num == 2) {
				pY++;
				if (map[pY][pX] == 9 || map[pY][pX] == 7) {
				return;
				}
				else if (map[pY][pX] == 3){ 
					ballY++;
					pY++;
					if (ballY < SIZE){
						map[ballY][ballX] = BALL; 
						map[pY][pX] = PLAYER;
					}
					else return;
				}
				else {
					pY++;
					if (pY < SIZE) {
						map[pY][pX] = PLAYER;
					}
				
			}
			
			}
			else if (num == 3) {
				pX--;
				if (map[pY][pX] == 9 || map[pY][pX] == 7) {
					return;
					}
					else if (map[pY][pX] == 3){ 
						ballY++;
						pY++;
						if (ballY < SIZE){
							map[ballY][ballX] = BALL; 
							map[pY][pX] = PLAYER;
						}
						else return;
					}
					else {
						pY++;
						if (pY < SIZE) {
							map[pY][pX] = PLAYER;
						}
			}
			
		}	*/
		
		
		
		
		while (true) {
			// 화면 출력
			System.out.println("---------------");
			for (int i = 0; i < SIZE; i++) {
				for (int j = 0; j < SIZE; j++) {
					if (map[i][j] == PLAYER) 	System.out.print("옷 ");
					else if(map[i][j] == WALL) 	System.out.print("! ");
					else if(map[i][j] == GOAL) 	System.out.print("G ");
					else if(map[i][j] == BALL) 	System.out.print("* ");
					else 						System.out.print(map[i][j] + " ");
				}
				System.out.println();
			}
			System.out.println("---------------");
			
			// 골대에 공을 넣으면 게임 종료
			if (ballY == goalY && ballX == goalX) {
				System.out.println("게임종료");
				break;
			}
			
			System.out.print("상(1)하(2)좌(3)우(4) 입력 : ");
			int move = scan.nextInt();
		
			// 현재위치 저장하기
			int yy = pY;
			int xx = pX;
			
			if		(move == 1) yy--;
			else if (move == 2) yy++;
			else if (move == 3) xx--;
			else if (move == 4) xx++;
			
			// 예외처리
			if (SIZE <= xx || xx < 0) 					 	continue;
			if (SIZE <= yy || yy < 0) 					 	continue;
			if (map[yy][xx] == WALL || map[yy][xx] == GOAL) continue;
			
			// 공을 만나면
			if (map[yy][xx] == BALL) {
				int yyy = ballY;
				int xxx = ballX;
				if		(move == 1) yyy--;
				else if (move == 2) yyy++;
				else if (move == 3) xxx--;
				else if (move == 4) xxx++;

				// 예외처리
				if (SIZE <= xxx || xxx < 0) continue;
				if (SIZE <= yyy || yyy < 0) continue;
				if (map[yyy][xxx] == WALL)  continue;
				
				// 공 이동시키기
				map[ballY][ballX] = 0; //
				ballY = yyy;
				ballX = xxx;
				map[ballY][ballX] = BALL;
			}
			
			// 캐릭터 이동시키기
			map[pY][pX] = 0;// 이동시킨뒤 0 처리를 해야하기 때문에 인덱스값을 변수에 넣어야함.
			pY = yy;
			pX = xx;
			map[pY][pX] = PLAYER;
			
		}

		scan.close();
		
	}

}
